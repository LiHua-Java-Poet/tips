<template>
    ...
    <el-dialog title="DOCX文件预览" :visible.sync="visibleDoc" :close-on-click-modal="false" center>
        <div v-show="!docxFinish" style="display: flex; justify-content: center;">
            请等待渲染完成
        </div>
        <div v-show="docxFinish" style="width: 100%;display: flex;justify-content: center;">
            <div ref="docx"></div>
        </div>
    </el-dialog>
    <el-dialog title="PDF文件预览" :visible.sync="visiblePdf" :close-on-click-modal="false" center width="180vh">
        <div style="width: 100%;display: flex;justify-content: center;">
            <iframe
                :src="visiblePdf"
                typeof="application/x-google-chrome-pdf"
                style="width: 100%;height: 900px"
            ></iframe>
        </div>
    </el-dialog>
    <el-dialog title="EXCEL文件预览" :visible.sync="visibleXlsx" :close-on-click-modal="false" center width="150vh">
        <div style="width: 100%;display: flex;justify-content: center;">
            <div v-html="excelView" class="excel-view" style="width: 80%;margin-left: 5%"></div>
        </div>
    </el-dialog>
    <el-dialog title="Txt文件预览" :visible.sync="visibleTxt" :close-on-click-modal="false" center width="100vh">
        <div style="width: 100%;display: flex;justify-content: center;">
            <pre>{{ txtContent }}</pre>  
        </div>
    </el-dialog>
...
<script>
    ...
    export default{
        ...
        methods:{
            itemClick(obj){
                if(obj.file_type==5){
                    this.docxFinish=false
                    this.visibleDoc=''
                    this.visibleDoc='http://localhost:8083/Preview/DocxBlob?file_id='+obj.file_id
                    axios({
                        method: "get",
                        ...
                        .then(({ data }) => {
                            docx.renderAsync(data, this.$refs.docx); // 渲染到页面预览
                            this.docxFinish=true
                        })
                        .catch(error=>{
                            console.info(error)
                            this.docxFinish=true
                ...
                if(obj.file_type==4){
                    this.visiblePdf=''
                    this.visiblePdf='http://localhost:8083/Preview/previewPdf?file_id='+obj.file_id
                }
                if(obj.file_type==6){
                    this.visibleXlsx=''
                    this.visibleXlsx='http://localhost:8083/Preview/previewXlsx?file_id='+obj.file_id
                    axios({
                            method: "get",
                            responseType: "arraybuffer", // 设置响应文件格式
                            url: this.visibleXlsx,
                        })
                        .then((res=>{
                            if (res) {
                ...
                if(obj.file_type==7){
                    this.visibleTxt=''
                    this.visibleTxt='http://localhost:8083/Preview/previewXlsx?file_id='+obj.file_id
                    axios({
                        method: "get",
                        responseType: "blob", // 设置响应文件格式
                        url: this.visibleTxt,
...
.excel-view td, .excel-view th {  
    border: 1px solid #636d7e; /* 设置边框 */  
